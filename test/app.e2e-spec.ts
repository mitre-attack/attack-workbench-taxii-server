...
const ENTERPRISE_ATTACK_COLLECTION_ID =
  "x-mitre-collection--402e24b4-436e-4936-b19b-2038648f489";
const OBJECT_ID = "attack-pattern--322bad5a-1c49-4d23-ab79-76d641794afa";

// We increase the timeout to accommodate the test on 'Get Objects' because it takes a while to retrieve the entire
// collection bundle
...
describe("TAXII 2.1 REST API", () => {
        ...
        .end((err, res) => {
          try {
            ...
            expect(res.body).toEqual({
              ...app.get(DiscoveryService).discover(),
            });
          } catch (err) {
            err.message = formatError(err, res);
            throw err;
  ...
  describe("CollectionController", () => {
    ...
    it("Get a Collection :: GET {api-root}/collections/{collection-id}/", (done) => {
      request(app.getHttpServer())
        .get(`/collections/${ENTERPRISE_ATTACK_COLLECTION_ID}/`)
        .set(commonHeaders)
        .end((err, res) => {
          try {
            expect(res.statusCode).toEqual(200);
    ...
    it("Get Object Manifests :: GET {api-root}/collections/{collection-id}/manifest/", (done) => {
      request(app.getHttpServer())
        .get(`/collections/${ENTERPRISE_ATTACK_COLLECTION_ID}/manifest/`)
        .set(commonHeaders)
        .end((err, res) => {
          try {
            expect(res.statusCode).toEqual(200);
    ...
    it("Get Objects :: GET {api-root}/collections/{collection-id}/objects/", (done) => {
      request(app.getHttpServer())
        .get(`/collections/${ENTERPRISE_ATTACK_COLLECTION_ID}/objects/`)
        .set(commonHeaders)
        .end((err, res) => {
          try {
            expect(res.statusCode).toEqual(200);
    ...
    it("Add Objects :: POST {api-root}/collections/{collection-id}/objects/", (done) => {
      request(app.getHttpServer())
        .post(`/collections/${ENTERPRISE_ATTACK_COLLECTION_ID}/objects/`)
        .set(commonHeaders)
        .end((err, res) => {
          try {
    ...
    it("Pagination is working", (done) => {
      request(app.getHttpServer())
        .get(`/collections/${ENTERPRISE_ATTACK_COLLECTION_ID}/objects/?limit=5`)
        .set(commonHeaders)
        .end((err, res) => {
          try {
            expect(res.statusCode).toEqual(200);
