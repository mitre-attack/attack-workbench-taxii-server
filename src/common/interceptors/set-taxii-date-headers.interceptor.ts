            ...
            map(data => {
                ...
                if (data.items) {
                    ...
                    switch (this.type) {
                        case TaxiiDateFrom.ENVELOPE: {
                            ...
                            if (stixObjects.length >= 1) {
                                ...
                                this.logger.debug(`Not setting headers 'x-taxii-date-added-first' or 'x-taxii-date-added-last' because returned envelope resource does not contain any objects`, this.constructor.name);
                            }
                            break;
                        }
                        case TaxiiDateFrom.MANIFEST: {
                            ...
                            if (manifestRecords.length >= 1) {
                                ...
                                this.logger.debug(`Not setting headers 'x-taxii-date-added-first' or 'x-taxii-date-added-last' because returned manifest resource does not contain any objects`, this.constructor.name);
                            }
                            break;
                        }
                        case TaxiiDateFrom.VERSIONS: {
                            ...
                            if (versions.length >= 1) {
                                ...
                                this.logger.debug(`Not setting headers 'x-taxii-date-added-first' or 'x-taxii-date-added-last' because returned versions resource does not contain any strings`);
                            }
                            break;
                        }
                    }
                    ...
                    if (addedFirst && addedLast) {
                        ...
                        this.logger.debug(`Set response header 'x-taxii-date-added-first: ${addedFirst}`, this.constructor.name);
                        res.setHeader('X-TAXII-Date-Added-Last', addedLast);
                        this.logger.debug(`Set response header 'x-taxii-date-added-last: ${addedLast}`, this.constructor.name);
