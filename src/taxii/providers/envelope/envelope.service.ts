...
export class EnvelopeService {
    ...
    async findByCollection(
        ...
        limit?: number,
        next?: number,
        match?: MatchDto) : Promise<EnvelopeDto>
    {
        const filters = new ObjectFiltersDto({ collectionId, addedAfter, limit, match });

        // First, get all of the STIX objects. Once acquired, we will paginate them into envelopes.
        let stixObjects: StixObjectPropertiesInterface[] = await this.objectsService.findByCollection(filters);
