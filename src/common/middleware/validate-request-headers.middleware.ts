...
export class ValidateRequestHeadersMiddleware implements NestMiddleware {
    ...
    private validate(req: Request) {
        ...
        this.logger.debug(`[${ctx["x-request-id"]}] Request object contains Accept header: ${acceptHeader}`);
        this.logger.debug(`[${ctx["x-request-id"]}] Request Accept header (${acceptHeader}) is ${isValid? 'VALID' : 'INVALID'}`);
        ...
        if (!isValid){
            ...
            this.logger.error(`[${ctx["x-request-id"]}] The request includes an Accept header but the header is either missing the ' +
    ...
    private isValidAcceptHeader(acceptHeader: string): boolean {
        ...
        acceptHeader.split(',').forEach((mediaType: string) => {
            ...
            allDefinedMediaTypes.push(
                this.parseMediaType(mediaType)
            );
        });
        /************************************************************************
    ...
    private isValidMediaType(mediaType: MediaTypeObject): boolean {
        ...
        if (typeMatch && subTypeMatch) {
            ...
            if (mediaType.version) {
                ...
                return mediaType.optionKey == MEDIATYPE_OPTION_KEY_FIELD
    ...
    private parseMediaType(mediaType: string): MediaTypeObject {
        ...
        if (typeAndSubType[1]) {
            ...
            if (subTypeAndVersion[1]) {
                ...
                mediaTypeObject.optionValue = parsedVersion[1] ? parsedVersion[1] : null;
            }
        }
