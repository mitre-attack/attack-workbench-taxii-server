...
// ** providers ** //
import {CollectionService, EnvelopeService, ManifestService, VersionService} from "src/taxii/providers";
...
export class CollectionsController {
    ...
    {
        this.logger.debug(`Received request for a single collection with options { collectionId: ${collectionId} }`, this.constructor.name);
        return await this.collectionService.findOne(collectionId);
    }


    @ApiOkResponse({
        description: "Manifest information about the contents of a specific collection",
    ...
    {
        this.logger.debug(`Received request for object manifests with options { collectionId: ${collectionId}, addedAfter: ${addedAfter}, limit: ${limit}, next: ${next}, match: ${match} }`, this.constructor.name);
        return await this.manifestService.getManifestsByCollection(collectionId, addedAfter, limit, next, match);
    }
    ...
    {
        this.logger.debug(`Received request for objects with options { collectionId: ${collectionId}, addedAfter: ${addedAfter}, limit: ${limit}, next: ${next}, match: ${match} }`, this.constructor.name);
        return await this.envelopeService.findByCollection(collectionId, addedAfter, limit, next, match);
    }
    ...
    {
        this.logger.debug(`Received request for an object with options { collectionId: ${collectionId}, objectId: ${objectId} }`, this.constructor.name);
        return await this.envelopeService.findByObjectId(collectionId, objectId, addedAfter, limit, match);
    }
    ...
    {
        this.logger.debug(`Received request for object versions with options { collectionId: ${collectionId}, objectId: ${objectId}, addedAfter: ${addedAfter}, limit: ${limit}, next: ${next}, match: ${match} }`, this.constructor.name);
        return this.versionsService.findObjectVersions(collectionId, objectId, addedAfter, limit, next, match);
    }
}
