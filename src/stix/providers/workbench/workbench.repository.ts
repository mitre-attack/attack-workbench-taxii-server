...
import {TaxiiBadRequestException, TaxiiNotFoundException, TaxiiServiceUnavailableException} from "src/common/exceptions";
...
@Injectable()
export class WorkbenchRepository extends StixRepositoryAbstract implements StixRepositoryInterface {
    ...
    private async fetchHttp(url: string) : Promise<any> {
        ...
        try {
            ...
            if (err.response) {
                ...
                this.logger.error(`A request was made to ${url} but the server responded with status code ${err.response.status}`, this.constructor.name);

                if (err.response.status >= 400 && err.response.status <= 499) {
                    throw new TaxiiBadRequestException({
                        title: "STIX Objects Not Found",
                        description: "The TAXII server made a request to the STIX server but the target responded with a 4xx status code. The requested STIX object may not be available. Please verify that your request is correct and contact the TAXII server administrator for help.",
                    });
                }
                ...
                if (err.response.status >= 500 && err.response.status <= 599) {
                    throw new TaxiiServiceUnavailableException({
                        title: "STIX Objects Not Found",
                        description: "The TAXII server made a request to the STIX server but the target responded with a 5xx status code. The STIX server may be temporarily unavailable. Please contact the TAXII server administrator and/or try again later.",
                    });
                }

            }
            else if (err.request) {
                this.logger.error(`A request was made to ${url} but no response was received`, this.constructor.name);
                ...
                throw new TaxiiServiceUnavailableException({
                    title: "STIX Objects Not Found",
                    description: "The TAXII server made a request to the STIX server but no response was received. Please contact the TAXII server administrator and/or try again later."
                });

            }
            else {
                // Something happened in setting up the request that triggered an Error
                this.logger.error(`Something happened in setting up the request to ${url} that triggered Error: ${err.message}`, this.constructor.name);

                throw new TaxiiNotFoundException({
                    title: "STIX Objects Not Found",
                    description: "Something happened while setting up the HTTP request to the STIX server. Please contact the TAXII server administrator."
                });
            }
        }
    }
    ...
    async getCollections(collectionId?: string) : Promise<WorkbenchCollectionDto[]> {
        ...
        response.forEach(collection => {
            ...
            const wbCollection = plainToClass(
     ...
     */
    async getAnObject(collectionId: string, stixId: string, versions: boolean = false) : Promise<WorkbenchStixObjectDto[]> {
