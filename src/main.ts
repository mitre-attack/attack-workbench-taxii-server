...
async function bootstrap() {
    ...
    try {
        ...
        if (tempConfigService.HTTPS_ENABLED) {
            ...
            console.log(`Starting the TAXII server at https://${tempConfigService.APP_ADDRESS}:${tempConfigService.APP_PORT}...`);
            https.createServer(httpsOptions, server).listen(tempConfigService.APP_PORT);

        } else {
            // HTTP (insecure)
            console.log(`Starting the TAXII server at http://${tempConfigService.APP_ADDRESS}:${tempConfigService.APP_PORT}...`);
