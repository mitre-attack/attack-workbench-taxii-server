...
export class CollectionService {
    ...
    async findAll(): Promise<TaxiiCollectionsDto> {
        ...
        let taxiiCollections: TaxiiCollectionsDto = await this.stixCollectionsRepo.findAll();


        if (!taxiiCollections) {
            throw new TaxiiNotFoundException({
                title: "Stix Collections Not Found",
                description: "The STIX Collections Repo did not return any collection objects. Ensure that STIX data exists in the stix.",
            });
        }

        this.logger.debug(`Retrieved ${taxiiCollections.length} STIX Collections from the repository`, this.constructor.name);
    ...
    async findOne(id: string): Promise<TaxiiCollectionDto> {
        ...
        this.logger.debug(`Retrieved STIX Collection ${taxiiCollection.id} from the repository`, this.constructor.name);
