...
export class ObjectService {
     ...
     * @param filters
     */
    async findByCollection(filters?: ObjectFiltersDto): Promise<StixObjectPropertiesInterface[]> {
        ...
        const stixBundle: StixBundleInterface = await this.stixObjectRepo.findByCollection(filters.collectionId);
        ...
        this.logger.debug(`Successfully retrieved ${stixObjects.length} STIX objects`, this.constructor.name);
        ...
        for (let i = 0; i < stixObjects.length; i++) {
            if (stixObjects[i].type === "x-mitre-collection"){
                ...
                this.logger.debug('Removed all instances of objects whose type equals x-mitre-collection', this.constructor.name);
    ...
    async findOne(
        collectionId: string,
        objectId: string,
        filters?: ObjectFiltersDto): Promise<StixObjectPropertiesInterface[]>
    {
        // Retrieve the STIX object from the connected STIX repository.
        const stixObject: StixObjectInterface[]
            = await this.stixObjectRepo.findOne(collectionId, objectId, filters.versions);
